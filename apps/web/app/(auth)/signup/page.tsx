...
function SignupForm() {
  ...
  useEffect(() => {
    ...
    if (intent === 'store') setRole('seller');
    else if (r === 'seller' || r === 'affiliate' || r === 'buyer' || r === 'studio') setRole(r);
  }, [searchParams]);

  const destination = useMemo(
    () => safeAuthRedirect(redirectParam, role === 'studio' ? '/studio' : '/dashboard'),
    [redirectParam, role],
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    setSuccess('');
    if (role === 'seller' && !storeConsent) {
      setError('Please grant store access below so we can create your seller storefront.');
      return;
    }
    ...
    const confirmRedirect = `${window.location.origin}/auth/callback?redirect=${encodeURIComponent(destination)}`;
    ...
    const { data, error: authError } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: {
          ...
          name: fullName,
          role,
          store_name: role === 'seller' ? storeName.trim() : undefined,
          store_permission_granted: role === 'seller' ? true : undefined,
        },
      },
    });
    ...
    setSuccess(
      'Check your email — we sent a confirmation link. After you confirm, you can sign in and your store will be set up if you chose seller.',
    );
    setLoading(false);
  }
  ...
  return (
    <div className="w-full max-w-md space-y-8">
      ...
      {success && (
        <div className="bg-green-50 border border-green-200 text-green-800 text-sm px-4 py-3 rounded-xl">{success}</div>
      )}
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {role === 'seller' && (
          <>
            ...
            <label className="flex items-start gap-3 cursor-pointer rounded-xl border border-outline/30 p-4 bg-white">
              <input
                type="checkbox"
                checked={storeConsent}
                onChange={(e) => setStoreConsent(e.target.checked)}
                className="mt-1 rounded border-outline accent-green-600"
              />
              <span className="text-sm text-on-surface leading-relaxed">
                I grant BengalStitch permission to create and manage my seller store, list products, and process orders on
                my behalf. I agree to the{' '}
      ...
      <p className="text-center text-sm text-on-surface-variant">
        ...
        <Link href={`/login?redirect=${encodeURIComponent(destination)}`} className="text-primary font-semibold hover:underline">
          Sign in
        </Link>
      </p>
    </div>
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-1 gradient-bg items-center justify-center p-16 relative overflow-hidden">
        ...
        <div className="relative z-10 text-white text-center max-w-md">
          ...
          <h2 className="editorial-headline text-4xl font-extrabold mb-4">Your brand, your store</h2>
          <p className="text-white/80 text-lg leading-relaxed">
            Sign up, grant store access once, and we create your storefront. You keep control of pricing and catalog.
          </p>
