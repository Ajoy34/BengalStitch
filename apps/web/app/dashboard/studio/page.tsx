...
export default function StudioPage() {
  ...
  const renderPreview = useCallback(async () => {
    ...
    try {
      ...
      ctx.globalAlpha = 1;
    } catch { /* base image failed - just show color */ }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="max-w-md text-center space-y-6">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900">BengalStitch Design Studio</h1>
          <p className="text-gray-500">Sign in to start designing custom products and launch your store.</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => router.push('/login?redirect=/dashboard/studio')} className="gradient-cta px-6 py-3 rounded-xl font-bold">
              Sign In
            </button>
            <Link href="/signup" className="px-6 py-3 rounded-xl border border-gray-200 font-bold text-gray-700 hover:bg-gray-50">
              Create Account
            </Link>
          </div>
        </div>
