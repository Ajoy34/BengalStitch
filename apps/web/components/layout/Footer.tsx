import Link from 'next/link';

const FOOTER_SECTIONS = {
  Explore: [
    { href: '/blog', label: 'Blog' },
    { href: '/academy', label: 'Academy' },
    { href: '/#how-it-works', label: 'How It Works' },
    { href: '/marketplace', label: 'Browse Products' },
  ],
  'Our Solutions': [
    { href: '/pricing', label: 'Pricing' },
    { href: '/marketplace', label: 'Online Stores' },
    { href: '/studio', label: 'Design Your Product' },
    { href: '/affiliates', label: 'Affiliate Program' },
  ],
  Policy: [
    { href: '/returns', label: 'Returns' },
    { href: '/privacy', label: 'Privacy Policy' },
    { href: '/terms', label: 'Terms of Service' },
    { href: '/copyright', label: 'Copyright Notice' },
  ],
  Support: [
    { href: '/about', label: 'About Us' },
    { href: '/contact', label: 'Contact Us' },
    { href: '/faq', label: 'FAQ' },
    { href: '/careers', label: 'Careers' },
  ],
};

const SOCIAL_LINKS = [
  { label: 'Facebook', href: '#', icon: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
  { label: 'Instagram', href: '#', icon: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z' },
  { label: 'YouTube', href: '#', icon: 'M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z' },
];

export function Footer() {
  return (
    <footer className="bg-gray-900 text-gray-300 pt-20 pb-8 px-6 md:px-8">
      <div className="container mx-auto max-w-7xl">
        {/* Top section */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 mb-16">
          <div className="col-span-2">
            <div className="text-2xl font-extrabold font-[family-name:var(--font-headline)] mb-4">
              <span className="text-green-400">BengalStitch</span>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed max-w-xs mb-6">
              One stop shop for on-demand products. High quality selection, low risk,
              seamless shopping experience guaranteed.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link href="/signup" className="gradient-cta px-5 py-2.5 rounded-xl text-sm font-bold">
                Create a Product
              </Link>
              <Link href="/signup" className="border border-gray-600 text-gray-300 hover:text-white hover:border-gray-400 px-5 py-2.5 rounded-xl text-sm font-bold transition-colors">
                Become a Seller
              </Link>
            </div>

            {/* Social icons */}
            <div className="flex gap-3 mt-6">
              {SOCIAL_LINKS.map((s) => (
                <a
                  key={s.label}
                  href={s.href}
                  className="w-9 h-9 bg-gray-800 hover:bg-green-600 rounded-lg flex items-center justify-center transition-colors"
                  aria-label={s.label}
                >
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                    <path d={s.icon} />
                  </svg>
                </a>
              ))}
            </div>
          </div>

          {Object.entries(FOOTER_SECTIONS).map(([title, links]) => (
            <div key={title}>
              <h4 className="text-sm font-bold text-white mb-4">{title}</h4>
              <ul className="space-y-2.5">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-gray-400 text-sm hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Payment methods */}
        <div className="border-t border-gray-800 pt-8 mb-8">
          <div className="flex flex-wrap items-center justify-center gap-4">
            {['bKash', 'Nagad', 'Rocket', 'SSLCommerz', 'Stripe', 'PayPal', 'Visa', 'Mastercard'].map((p) => (
              <span key={p} className="text-xs font-semibold text-gray-500 bg-gray-800 px-3 py-1.5 rounded-lg">{p}</span>
            ))}
          </div>
        </div>

        {/* Bottom bar */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-gray-800">
          <p className="text-gray-500 text-xs">
            &copy; {new Date().getFullYear()} BengalStitch. All rights reserved.
          </p>
          <p className="text-gray-500 text-xs">
            Made with love in Bangladesh
          </p>
        </div>
      </div>
    </footer>
  );
}
